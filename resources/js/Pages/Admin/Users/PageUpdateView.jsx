...
function PageCreateView({ auth }) {
  ...
  useEffect(()=>{
    updateCreatePageInputState('name',pageView?.data?.name );
    updateCreatePageInputState('email',pageView?.data?.email);
    updateCreatePageInputState('phone',pageView?.data?.phone);
    updateCreatePageInputState('role',pageView?.data?.role);
  ...
  const pageUpdateActionHandler =(event)=>{
    ...
    router.post(route('users.update', id), data, {
      onSuccess: (resp) => {
        toast(resp)
      }
    });
